...
class FileService extends BaseService{
    ...
    /*public function processSong(array $songData) {
        try {
            ...
            rename($songData['song_path'], $realPath . '/' . $name);
            $songUrl = '/static/' . $dir . '/' . $name;
            
            return json_encode(array('success' => true, 'song_url' => $songUrl, 'song_directory' => $dir));
    ...
    public function createZipFromAlbum($data) {
        $zip = new \ZipArchive();
        $zipName = "{$data['album_name']}.zip";
        $dir = "{$this->uploadDir}/{$data['album_dir']}";
        ...
        while($file = readdir($pointer)){
            if( $file != "." && $file != ".." && $file != $zipName){
                if(!is_dir("$dir/$file") ){
                    if(isset($data['song_names'][$file])) {
                        $zip->addFile("$dir/$file", $data['song_names'][$file]);    
                    } else {
